        ...
        if ( html_file[ index ] == '<' )
        {
            ...
            switch ( html_file[ index ] )
            {
                ...
                case 'a'  : //Link
                    ...
                    break;
                //script/style Want to completely skip these
                case 'S'  :
                case 's'  :
                    ...
                    { //was not script or style tag, treat as ordinary tag
                        goto DEFAULT;
                    }
                    break;
                ...
                case 't'  :
                    ...
                    {//was not <title, treat as ordinary tag
                        goto DEFAULT;
                    }
                    break;
                case 'H'  :
...
bool LinkFinder::find_open_tag( char *html_file )
{
    ...
    while ( index > 0 )
    {
        while ( index > 0 && index < file_length && html_file[ index ] != '>' )
        { //find tag
            if (html_file[index] == '>') {
                tag_found = true;
            }
        ...
        if ( !is_closing_tag )
        { //Got here, then must be opening tag
            num_opening++;
        }
        is_closing_tag = false; //reset
        if ( num_opening >= num_closing )
        { //check if tags are even
            return true;
        }
        index++;
    }
